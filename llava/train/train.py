...
class TrainingArguments(transformers.TrainingArguments):
    ...
    unfreeze_mm_vision_tower: bool = field(default=False)  # add by friedrichor, refer to Open-LLaVA-NeXT
...
class LazySupervisedDataset(Dataset):
    ...
    #                     return result
    #             image = expand2square(image, tuple(int(x*255) for x in processor.image_mean))
    #             image_size = image.size  # add by friedrichor, refer to Open-LLaVA-NeXT
    #             image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
    #         elif self.data_args.image_aspect_ratio == "anyres":  # add by friedrichor, refer to Open-LLaVA-NeXT
    #             image_size = image.size
    #             image = process_anyres_image(image, processor, self.data_args.image_grid_pinpoints) # torch.Size([5, 3, 336, 336])
    #         else:
    #             image_size = image.size  # add by friedrichor, refer to Open-LLaVA-NeXT
    #             image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
    #         sources = preprocess_multimodal(
    ...
    #     if 'image' in self.list_data_dict[i]:
    #         data_dict['image'] = image
    #         data_dict['image_size'] = image_size  # add by friedrichor, refer to Open-LLaVA-NeXT
    #     elif self.data_args.is_multimodal:
    #         # image does not exist in the data, but the model is multimodal
    #         crop_size = self.data_args.image_processor.crop_size
    #         data_dict['image'] = torch.zeros(3, crop_size['height'], crop_size['width'])
    #         data_dict['image_size'] = (crop_size['height'], crop_size['width'])  # add by friedrichor, refer to Open-LLaVA-NeXT
    #     return data_dict
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            if isinstance(images[0], list):
                # Ensure all images in the inner lists have the same shape for stacking
                stacked_images = [torch.stack(img, dim=0) if all(x is not None and x.shape == img[0].shape for x in img) else img for img in images]
...
#         if 'image' in instances[0]:
#             images = [instance['image'] for instance in instances]
#             image_sizes = [instance['image_size'] for instance in instances]  # add by friedrichor, refer to Open-LLaVA-NeXT
#             if all(x is not None and x.shape == images[0].shape for x in images):
#                 batch['images'] = torch.stack(images)
#             else:
#                 batch['images'] = images
#             batch['image_sizes'] = image_sizes  # add by friedrichor, refer to Open-LLaVA-NeXT
...
"""add by friedrichor, refer to Open-LLaVA-NeXT"""
